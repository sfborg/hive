...
// TestGuess covers the three tiers of the guess strategy (marker,
...
func TestGuess(t *testing.T) {
	...
	for _, c := range cases {
		...
		got := Guess(c.code, flat, flat.CanonicalSimple)
		if got != c.want {
			t.Errorf("Guess(%q, %q) = %q, want %q",
				c.code, c.name, got, c.want)
		}
	}
}

// TestGuessEmpty covers the "no useful signal" paths — empty parse
...
func TestGuessEmpty(t *testing.T) {
	...
	if got := Guess("ZOOLOGICAL", flat, ""); got != "" {
	...
	if got := Guess("UNKNOWN_CODE", flat, "Panthera"); got != "GENUS" {
