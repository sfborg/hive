...
func vocabComboSource(vocab *hive.Vocabulary, name string) comboboxSource {
...
func childRankComboSource(vocab *hive.Vocabulary, allowed []ui.ChildRank) comboboxSource {
	if len(allowed) == 0 {
...
func vocabLabel(t hive.VocabTerm) string {
	if t.Name != "" {
		return t.Name
	}
	if t.ID != "" {
...
// nomenclatural code (via hive.NomenCodePrefix) so a name under
...
func nomenComboSource(codeID string) comboboxSource {
	prefix := hive.NomenCodePrefix(codeID)
	return func(q string) tea.Cmd {
		return func() tea.Msg {
...
// references matching q, via hive.Archive.SearchReferences. Same async
// pattern as taxonComboSource. Requires at least 2 chars.
func referenceComboSource(a *hive.Archive) comboboxSource {
	return func(q string) tea.Cmd {
		return func() tea.Msg {
...
// are absent.
func referenceHitLabel(h hive.ReferenceHit) string {
...
// matching q, via hive.Archive.SearchTaxa. Runs in a goroutine (SQLite
...
func taxonComboSource(a *hive.Archive) comboboxSource {
	return func(q string) tea.Cmd {
		return func() tea.Msg {
			trimmed := strings.TrimSpace(q)
			if len(trimmed) < 2 {
