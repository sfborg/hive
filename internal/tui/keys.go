...
// values are derived from pkg/ui.Keymap() at construction time via
...
// must correspond to a real entry in pkg/ui.Keymap(), and any TUI
...
func bindingFor(action string) key.Binding {
	for _, s := range ui.Keymap() {
		...
		if len(keys) == 0 {
			panic(fmt.Sprintf("pkg/ui: action %q has no TUI keys but is referenced by the TUI keymap", action))
		}
	...
	panic(fmt.Sprintf("pkg/ui: unknown action %q referenced by the TUI keymap", action))
}
