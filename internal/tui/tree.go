...
func newTreeModel(a *hive.Archive) treeModel {
...
func hitToTreeNode(h hive.TaxonHit, depth int) treeNode {
	return treeNode{
		id:          h.ID,
		parentID:    h.ParentID,
		nameID:      h.NameID,
...
func (m treeModel) RevealCmd(id string) tea.Cmd {
	...
	return func() tea.Msg {
		...
		convert := func(hits []hive.TaxonHit, parentID string, depth, remaining int) []treeNode {
...
func (m treeModel) loadAllRemainingCmd(cursorAfterLoad int) tea.Cmd {
	...
	a := m.a
	return func() tea.Msg {
		// limit=0 → no LIMIT clause in hive.ListChildrenPage. Uncapped
...
func (m treeModel) View() string {
	...
	for i := start; i < end; i++ {
		...
		canonStyle := outer
		if hive.ItalicForRank(n.rank) && n.canonical != "" {
