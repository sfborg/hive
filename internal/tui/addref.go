...
// tabs, same in-process backing (hive.OpenAlex / hive.BHLnames /
// hive.ParseBibTeX / Tx.CreateReference) — the TUI just skips the HTTP
// layer entirely because it already holds the *hive.Archive.
//
// Tabs, in order:
//
...
type projectSearchDone struct {
	query string
	hits  []hive.ReferenceHit
	err   error
}
type bhlLookupDone struct {
	hits []hive.BHLnameHit
	err  error
}
...
func newAddRefModel(a *hive.Archive, actor, canonical, authors string, year int) addRefModel {
	q := textinput.New()
...
func (m *addRefModel) runBHLnamesCmd() tea.Cmd {
	...
	year := m.year
	return func() tea.Msg {
		hits, err := hive.BHLnames().LookupName(context.Background(),
...
func (m addRefModel) updateDOI(msg tea.KeyMsg) (addRefModel, tea.Cmd) {
	...
	case "enter":
		...
		return m, func() tea.Msg {
			ref, err := hive.OpenAlex().ResolveDOI(context.Background(), doi)
...
func (m *addRefModel) createAndPickCmd(ref coldp.Reference) tea.Cmd {
	...
	return func() tea.Msg {
		...
		if err != nil {
			...
			return refCreatedDone{id: newID, label: hive.ReferenceLabel(&ref)}
		}
		return refCreatedDone{id: newID, label: hive.ReferenceLabel(fresh)}
	}
}
...
func formatRefHit(h hive.ReferenceHit) string {
...
func formatBHLnameHit(h hive.BHLnameHit) string {
