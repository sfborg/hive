...
type detailModel struct {
	...
	// unrelated pane.
	pendingWarnings      []hive.ValidationWarning
	...
	// pkg/ui.ValidChildRanks (fetched once in EnterCreateModeCmd).
...
func newDetailModel(a *hive.Archive, editable bool, actor string) detailModel {
...
type detailLoadedMsg struct {
	...
	name        *coldp.Name
	parentLabel string
	warnings    []hive.ValidationWarning
	err         error
}
...
// display name via hive.TaxonRef (same server-side formatting the WUI
...
func (m *detailModel) EnterEditModeCmd() (tea.Cmd, bool) {
	...
	if m.nameEditable {
		...
		m.nameOriginals = nameFieldSnapshot{
			...
			StatusID:                  hive.NameRawStatus(m.name.ID),
			ReferenceID:               hive.PrimaryReferenceID(m.name.ReferenceID),
	...
	if m.nameEditable {
		...
		if refID := hive.PrimaryReferenceID(m.name.ReferenceID); refID != "" {
		...
		statusID := hive.NameRawStatus(m.name.ID)
		statusLabel := hive.NomenLabelFor(statusID)
		if statusLabel == "" {
...
func vocabLabelFor(v *hive.Vocabulary, name, id string) string {
	if id == "" {
		return ""
	}
	...
	return func() tea.Msg {
		...
		err := a.WithTx(ctx, func(tx *hive.Tx) error {
			...
			if writeName {
				...
				currentName.Rank = hive.ParseRank(nSnap.RankID)
				...
				// URIs survive — see hive.Tx.SetNameStatus.
...
func (m detailModel) renderFields() string {
	...
	if m.name != nil {
		...
		statusRaw := hive.NameRawStatus(m.name.ID)
		if lbl := hive.NomenLabelFor(statusRaw); lbl != "" {
			push("Nom status", lbl)
		} else {
...
// hive.ItalicForRank says so, dagger prefix for extinct taxa, authorship
...
// frontends stay visually aligned via the shared hive.ItalicForRank rule.
func (m detailModel) headerLine() string {
	...
	if hive.ItalicForRank(m.name.Rank.ID()) && canonical != "" {
...
func vocabTerms(v *hive.Vocabulary, name string) []hive.VocabTerm {
	if v == nil {
		return nil
	}
	switch name {
