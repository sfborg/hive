// Package tui implements the Bubble Tea TUI shared by `hive view` and
...
func Run(archivePath string, opts Options) error {
	...
	// via the same hive.WithActor(ctx, ...) plumbing the HTTP server uses.
...
func contextWithActor(actor string) context.Context {
	return hive.WithActor(context.Background(), actor)
}
