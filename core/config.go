...
// env values (see cmd/hive/main.go for the precedence order).
//
...
// ConfigPath resolves the on-disk location of hive's config file.
// Follows the XDG Base Directory spec on Linux, uses the platform-
// native equivalent elsewhere. Path is returned even when the file
// doesn't exist yet — callers seeding a fresh config Save into it.
//
// Precedence:
...
func ConfigPath() (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("core: user home dir: %w", err)
	}
	switch runtime.GOOS {
	case "darwin":
...
// returns a zero-value Config so callers see empty strings for every
// field and can decide what to fall back to (flag > env > empty).
//
...
func LoadConfig() (*Config, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("core: read config %s: %w", path, err)
	}
	var c Config
...
// SetIdentity records the merged identity for the process. Called
// once from main.go after resolving flag > env > config precedence.
// Safe to call multiple times but not concurrent-safe — should only
// happen during startup before any goroutines read it.
func SetIdentity(id Identity) { currentIdentity = id }

// CurrentIdentity returns the merged identity set by SetIdentity, or
// a zero-value Identity if SetIdentity was never called (e.g. in
// tests). Client packages that need the ORCID actor for auditing or
// the OpenAlex email for polite-pool headers read from here.
func CurrentIdentity() Identity { return currentIdentity }

// SaveConfig writes the config to disk, creating parent directories as
...
func SaveConfig(c *Config) error {
	...
	if err := os.MkdirAll(filepath.Dir(path), 0o700); err != nil {
		return fmt.Errorf("core: create config dir: %w", err)
	}
	...
	// fields set; that's readable but confusing. Emit a helpful
	// header comment so a curator opening the file for the first
	// time sees what it's for.
