...
// RankGuess returns a suggested sfga rank ID for a name, given its
...
func RankGuess(codeID string, p parsed.ParsedFlat, canonical string) string {
	...
	// 3. Cardinality fallback — pure structural inference from how
	//    many words the parser saw.
	...
	case 2:
		return "SPECIES"
	case 3:
		// SUBSPECIES is the safe zoological default; botany often
		// uses VARIETY / FORM without a marker but there's no way
		// to tell from cardinality alone, so pick the most common.
		return "SUBSPECIES"
	}
	return ""
}
