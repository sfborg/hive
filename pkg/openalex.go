...
import (
	...
	"github.com/sfborg/hive/pkg/openalex"
)
...
// process identity's OpenAlexEmail (see config.CurrentIdentity, set by
...
func OpenAlex() *openalex.Client {
	if openAlexInstance == nil {
		openAlexInstance = openalex.New(config.CurrentIdentity().OpenAlexEmail)
	}
