...
// TestArticle covers the common @article path with author list,
...
// TestBook covers @book with editor, publisher/place, ISBN, year-only
// Issued.
...
// TestChapter covers @incollection using booktitle for the container
// title.
...
// TestDOIStripsPrefix — DOI field is often pasted as a full URL.
func TestDOIStripsPrefix(t *testing.T) {
...
// TestMalformed covers the parse-error path so callers see a wrapped
// ErrParse (for errors.Is checks).
func TestMalformed(t *testing.T) {
	...
	for i, s := range cases {
		...
		if !errors.Is(err, ErrParse) {
			t.Errorf("case %d: err %v is not ErrParse", i, err)
		}
	}
}
...
// TestMonthCoercion covers month-token normalization: three-letter,
// full, numeric, unknown.
func TestMonthCoercion(t *testing.T) {
	cases := map[string]string{
...
// TestAuthorSplit checks the " and " → "; " conversion and preservation
// of braced groups (e.g. "{van der Waals, K.}").
func TestAuthorSplit(t *testing.T) {
	...
	for in, want := range cases {
		if got := authorToColdp(in); got != want {
			t.Errorf("authorToColdp(%q) = %q, want %q", in, got, want)
		}
	}
}

// TestMultipleEntriesTakesFirst — paste with a stray second entry
// shouldn't blow up; first entry wins per the doc.
func TestMultipleEntriesTakesFirst(t *testing.T) {
	src := `
