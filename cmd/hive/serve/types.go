...
// mirror core.Metadata but with wire-friendly JSON tags. Pointer-
...
func metadataToAPI(m *core.Metadata) apiMetadata {
	return apiMetadata{
		ID:              m.ID,
		DOI:             m.DOI,
		Title:           m.Title,
...
func applyMetadataPatch(m *core.Metadata, p apiMetadataPatch) {
	if p.DOI != nil {
		m.DOI = *p.DOI
	}
	if p.Title != nil {
...
// core.TaxonHit but with wire-friendly JSON keys and pointer-optional
// fields. `label` is the server-rendered display (see core.BuildLabel);
...
// apiTaxon and any other resolved reference. See core.BuildLabel.
...
func referenceHitToAPI(h core.ReferenceHit) apiReferenceHit {
...
// omitted — gets filled from gnparser by core.CreateName's fill-from-
...
// core.CreateName. Enum fields go through the empty-safe helpers so
...
func (b createTaxonBody) toColdpName() coldp.Name {
	return coldp.Name{
		...
		Rank:                      core.ParseRank(b.Rank),
		Code:                      nomcode.New(b.Code),
		Status:                    core.ParseNomStatus(b.Status),
...
func bhlnameHitToAPI(h core.BHLnameHit) apiBHLnameHit {
...
func applyNamePatch(n *coldp.Name, p apiNamePatch) {
	...
	if p.Rank != nil {
		n.Rank = core.ParseRank(*p.Rank)
	}
	if p.Code != nil {
		n.Code = nomcode.New(*p.Code)
	}
	if p.Status != nil {
		n.Status = core.ParseNomStatus(*p.Status)
	}
	if p.Gender != nil {
...
func hitToAPI(h core.TaxonHit) apiTaxonHit {
	return apiTaxonHit{
		ID:          h.ID,
		ParentID:    h.ParentID,
		NameID:      h.NameID,
...
func nameHitToAPI(h core.NameHit) apiNameHit {
	return apiNameHit{
		ID:         h.ID,
...
// last read it (via core.NameRawStatus), falling back to the enum's
// stringified form for legacy CoLDP-generalized values. See
// core/nomen.go for the sidecar cache.
func statusOf(n *coldp.Name) string {
	if raw := core.NameRawStatus(n.ID); raw != "" {
		return raw
	}
	return n.Status.ID()
}
...
func nameToAPI(n *coldp.Name) apiName {
	return apiName{
		...
		Authors:              n.Authors,
		GnID:                 n.GnID,
		ReferenceID:          core.PrimaryReferenceID(n.ReferenceID),
...
// synonymHitToAPI converts a core.SynonymHit (with pre-built Label) into
...
func synonymHitToAPI(h core.SynonymHit) apiSynonym {
	return apiSynonym{
		ID:         h.ID,
		TaxonID:    h.TaxonID,
		NameID:     h.NameID,
